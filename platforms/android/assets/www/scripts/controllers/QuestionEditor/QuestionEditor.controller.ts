...
angular.module("tApp").controller("QuestionEditor", function($scope, QuestionService, $rootScope, $filter) {
  ...
  */
  ...
  $rootScope.$on("QuestionAddedEvent", function(evt, data) {
    if (data.question && data.question.questionId <= 0) {
      console.log("Question Added with invalid ID! (probably not stored on server): " + data.question + ", reason: " + data.exception);
    } else {
      console.log("Question Added: " + data.question + ", reason: " + data.exception);
    }
    // ui-bootstrap to populate a ui-alert box
  })

  $rootScope.$on("QuestionAddFailedEvent", function(evt, data) {
    console.log("Failed to add question!: " + data.question + ", reason: " + data.exception);
    // ui-bootstrap to populate a ui-alert box
  });
